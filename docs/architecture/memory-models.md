...
This document provides a deep, definitive architectural reference on the memory models, hardware cache mechanics, kernel-bypass interactions, and zero-allocation strategies implemented across **AWP (Async Worker Pool & Ultra-Low-Latency HFT Engine)** in **Zig 0.16**, **C11**, and **Rust (`awp-zig-rs`)** on **Linux (x86_64 / aarch64)** and **macOS (Apple Silicon)**.
...
- [6. Lock-Free Cache-Aligned Rings & 64-Byte POD Primitives](#6-lock-free-cache-aligned-rings--64-byte-pod-primitives)
  - [6.1 False Sharing Elimination (64-byte Cache-Line Padding)](#61-false-sharing-elimination-64-byte-cache-line-padding)
  - [6.2 64-Byte POD Cacheline Structs (BookUpdate64, Trade64, OrderSignal64)](#62-64-byte-pod-cacheline-structs-bookupdate64-trade64-ordersignal64)
...
To completely eliminate `malloc`/`free` while maintaining strict spatial locality, the C engine implements **Page-Aligned Ring Slabs**.
...
## 6. Lock-Free Cache-Aligned Rings & 64-Byte POD Primitives
...
```zig
// Zig 0.16 SPSC Ring
write_idx: std.atomic.Value(usize) align(64),
cached_read_idx: usize,

read_idx: std.atomic.Value(usize) align(64),
cached_write_idx: usize,
```

### 6.2 64-Byte POD Cacheline Structs (`BookUpdate64`, `Trade64`, `OrderSignal64`)
By packing financial events into exactly 64 bytes (`align(64)`), each tick occupies exactly **one L1 cacheline**:
* **`BookUpdate64`**: Top-of-book market update (64B).
* **`Trade64`**: Execution trade report (64B).
* **`OrderSignal64`**: Fast-path order signal (64B).

```zig
pub const OrderSignal64 = extern struct {
    timestamp_ns: u64 align(64) = 0,
    ingress_ts_ns: u64 = 0,
    order_id: u64 = 0,
    price: f64 = 0,
    qty: f64 = 0,
...
The Rust FFI crate (`awp-zig-rs`) wraps the C ABI in safe, idiomatic Rust RAII semantics with zero overhead.
...
┌───────────────────────────────────▼────────────────────────────────────┐
│  2. User In-Place Write: guard.write_struct(&update)                   │
...
| Dimension | Standard `malloc`/`free` | Linux HFT Environment | C11 Engine (`libawp`) | Zig 0.16 Engine (`awp_zig`) | Rust Bindings (`awp-zig-rs`) |
| :--- | :--- | :--- | :--- | :--- | :--- |
...
| **TLB Optimization** | Random 4KB pages | **2MB / 1GB HugePages + `MAP_POPULATE`** | **4096-byte Page Alignment** | **2MB HugePage Slab** | Inherited from C ABI |
...
| **Trading Reactor Tick-to-Trade** | N/A | **Sub-microsecond ($< 300\text{ ns}$)** | N/A | **247.78 ns** (4.04 M ticks/s) | **253.16 ns** (3.95 M ticks/s) |
| **Variable-Length BipRing Throughput** | ~1.5 M pkt/s | **Zero Split-Wrap Copy** | N/A | **14.21 M pkts/s** (~8.52 GB/s) | **13.80 M pkts/s** (~8.28 GB/s) |
